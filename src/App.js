...
export default function App() {
  ...
  const [quickAmts, setQuickAmts] = useState(() => {
    ...
    try { const arr = JSON.parse(raw); return Array.isArray(arr) && arr.length ? arr : ["5.00", "10.00", "20.00", "50.00"]; }
    catch { return ["5.00", "10.00", "20.00", "50.00"]; }
  });
  const [tipPresets, setTipPresets] = useState(() => {
    const raw = localStorage.getItem("tipPresets");
    if (!raw) return [0, 10, 15, 20];
    try { const arr = JSON.parse(raw); return Array.isArray(arr) && arr.length ? arr : [0, 10, 15, 20]; }
    catch { return [0, 10, 15, 20]; }
  });
  ...
  function resetForm() {
    setAmount("25.00"); setTipPct(0); setTipMode("percent"); setTipFixed("");
    setPayerId("walk-in"); setCustomerEmail(""); setStartResult(null); setError("");
  ...
  async function sendEmail() {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/payments/${emailTargetId}/email`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ to_email: emailAddress.trim() }),
      });
      const raw = await r.text();
      let data; try { data = JSON.parse(raw); } catch { data = { raw }; }
      if (!r.ok) throw new Error(data?.error || data?.detail || `HTTP ${r.status}: ${String(raw).slice(0, 140)}`);
      alert(`Receipt sent to ${emailAddress.trim()}`); cancelEmailForm();
    } catch (e) {
      console.error("sendEmail error:", e); alert(`Failed to send: ${e.message || e}`);
  ...
  async function recheck(paymentId) {
    try {
      ...
      alert(`Status: ${data.state || "unknown"}${data.confirmed ? " (confirmed)" : ""}`); fetchPayments();
    } catch (e) { console.error("recheck error", e); alert(`Re-check failed: ${e.message || e}`); }
  }
  ...
  function exportFilteredCsv() {
    try {
      const headers = [
        "created_at","payment_id","order_id","invoice_amount","invoice_currency","crypto_amount","currency",
        "state","status","cashier","tip_amount","tip_percent","tip_mode","customer_email"
      ];
        ...
        <div className="card-body row">
          ...
          <div className="row cols-3">
            ...
            <div>
              ...
              <input className="input" type="text" value={payerId} onChange={(e) => setPayerId(e.target.value)} placeholder="walk-in" />
            </div>
          </div>
          ...
          <div className="row cols-3">
            <div>
              ...
              <input className="input" type="text" value={cashier} onChange={(e) => setCashier(e.target.value)} placeholder="Cashier" />
            </div>
            <div>
              <label className="label">Customer email (optional)</label>
              <input className="input" type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="you@example.com" />
            </div>
          </div>
        ...
        <div className="card-body">
          <div className="input-group" style={{ flexWrap: "wrap" }}>
            <div className="input-group">
              ...
              <input className="input" type="date" id="rangeStart" defaultValue={monthStartISO()} />
            </div>
            <div className="input-group">
              <label className="label" style={{ margin: 0 }}>End</label>
              <input className="input" type="date" id="rangeEnd" defaultValue={todayISO()} />
            </div>
